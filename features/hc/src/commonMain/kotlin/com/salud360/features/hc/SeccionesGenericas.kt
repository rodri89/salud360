...
object SeccionesGenericas {
    ...
    val conductas = SeccionDef(C.CONDUCTAS, "Conductas", TipoSeccion.TEXTO)
    val observaciones = SeccionDef(C.OBSERVACIONES, "Observaciones", TipoSeccion.TEXTO)
    val nota = SeccionDef(C.NOTA, "Nota", TipoSeccion.TEXTO)
    val plan = SeccionDef(C.PLAN, "Plan", TipoSeccion.TEXTO)
    val resumen = SeccionDef(C.RESUMEN, "Resumen", TipoSeccion.TEXTO)
    val subjetivo = SeccionDef(C.DATOS_SUBJETIVOS, "Datos subjetivos", TipoSeccion.TEXTO)
    val objetivo = SeccionDef(C.DATOS_OBJETIVOS, "Datos objetivos", TipoSeccion.TEXTO)
    val medicacion = SeccionDef(C.MEDICACION_ACTUAL, "Medicación actual", TipoSeccion.TEXTO, icono = "medicacion")
    val habitos = SeccionDef("habitos", "Hábitos", TipoSeccion.TEXTO)
    ...
    val interconsulta = SeccionDef(C.INTERCONSULTA, "Interconsultas", TipoSeccion.REGISTROS, registroTipo = "interconsulta", icono = "registros")
    ...
    val screening = SeccionDef("screening", "Screening", TipoSeccion.REGISTROS, registroTipo = "screening", inicialmenteExpandida = false)
